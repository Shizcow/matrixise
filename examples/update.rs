...
fn main() {
    ...
    for i in 0..10 {
	screen.push(Message::new_simple(&("M".to_string()+&i.to_string()), "+0", COLOR_PAIR(COLOR_PAIR_WHITE), i));
    }
	...
	for i in 0..10 {
	    screen.push_update(Message::new_simple(&("M".to_string()+&i.to_string()), &("+".to_string()+&j.to_string()), COLOR_PAIR(COLOR_PAIR_WHITE), i)); // update prexisting messages
	}
	j += 1;
    }
